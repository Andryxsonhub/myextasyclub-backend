...
// --- ★★★ ATUALIZADO (Rota GET /online agora ordena por 'destacadoAte') ★★★ ---
...
// ... (Todas as funções auxiliares 's3Client', 'storage', 'upload', 'escapeSVG', 'createWatermarkSvg', 'addWatermark', 'uploadToS3', 'deleteFromS3' permanecem exatamente iguais) ...
// ======================
...
router.put('/profile/avatar', checkAuth, upload.single('avatar'), async (req, res) => {
    // ... (Código PUT original - sem alteração) ...
    try {
...
// ======================
// ROTAS DE PERFIL E MÍDIA (OK)
// ======================
router.get('/profile', checkAuth, async (req, res) => {
    // ... (Código GET /profile original - sem alteração) ...
    try {
...
router.get('/photos', checkAuth, async (req, res) => {
    // ... (Código GET /photos original - sem alteração) ...
    try {
        ...
        console.error("Erro ao buscar as fotos do usuário logado:", error);
        res.status(500).json({ message: "Erro interno ao buscar fotos." });
    }
});
router.get('/videos', checkAuth, checkPlanAccess(['mensal', 'anual']), async (req, res) => {
    // ... (Código GET /videos original - sem alteração) ...
    try {
        const userId = req.user.userId;
        ...
        console.error("Erro ao buscar os vídeos do usuário logado:", error);
        res.status(500).json({ message: "Erro interno ao buscar vídeos." });
    }
});
router.get('/profile/:id', checkAuth, async (req, res) => {
    // ... (Código GET /profile/:id original - sem alteração) ...
    try {
...
router.get('/:id/photos', checkAuth, async (req, res) => {
    // ... (Código GET /:id/photos original - sem alteração) ...
    try {
        const userId = parseInt(req.params.id, 10);
        if (isNaN(userId)) { return res.status(400).json({ message: "ID de usuário inválido." }); }
        ...
        console.error(`Erro ao buscar as fotos do usuário ${req.params.id}:`, error);
        res.status(500).json({ message: "Erro interno ao buscar fotos." });
    }
});
router.get('/:id/videos', checkAuth, checkPlanAccess(['mensal', 'anual']), async (req, res) => {
    // ... (Código GET /:id/videos original - sem alteração) ...
    try {
        const userId = parseInt(req.params.id, 10);
        if (isNaN(userId)) { return res.status(400).json({ message: "ID de usuário inválido." }); }
        ...
        console.error(`Erro ao buscar os vídeos do usuário ${req.params.id}:`, error);
        res.status(500).json({ message: "Erro interno ao buscar vídeos." });
    }
});
...
// ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
// ★★★ CORREÇÃO: Rota GET /online (Ordenação por Destaque) ★★★
// ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
    ...
    try {
        ...
        const onlineUsers = await prisma.user.findMany({
            ...
            select: { 
                ...
                name: true, 
                username: true, 
                destacadoAte: true, // ★ Seleciona o campo
                profile: { select: { avatarUrl: true, gender: true } } 
            },
            // ★ Ordena por Destaque (desc) e DEPOIS por LastSeen (desc)
            orderBy: [
                { destacadoAte: 'desc' }, 
                { lastSeenAt: 'desc' }
            ], 
        ...
        const formattedUsers = onlineUsers.map(user => ({ 
            ...
            // ★ Envia o campo para o frontend
            destacadoAte: user.destacadoAte 
        }));
...
router.put('/profile', checkAuth, async (req, res) => {
    // ... (Código PUT /profile original - sem alteração) ...
    try {
...
router.get('/search', checkAuth, async (req, res) => {
    // ... (Código GET /search original - sem alteração) ...
    try {
        const { q } = req.query;
        let whereClause = {
             status: 'ativo'
...
router.post('/search/advanced', checkAuth, async (req, res) => {
    // ... (Código POST /search/advanced original - sem alteração) ...
    try {
        const {
            searchTerm,
            location,
...
router.delete('/photos/:id', checkAuth, async (req, res) => {
    // ... (Código DELETE /photos/:id original - sem alteração) ...
    try {
...
router.delete('/videos/:id', checkAuth, async (req, res) => {
    // ... (Código DELETE /videos/:id original - sem alteração) ...
    try {
...
router.get('/:id/followers', checkAuth, async (req, res) => {
    // ... (Código GET /:id/followers original - sem alteração) ...
    try {
         const userId = parseInt(req.params.id, 10);
         if (isNaN(userId)) { return res.status(400).json({ message: "ID de usuário inválido." }); }
...
router.get('/:id/following', checkAuth, async (req, res) => {
    // ... (Código GET /:id/following original - sem alteração) ...
    try {
         const userId = parseInt(req.params.id, 10);
         if (isNaN(userId)) { return res.status(400).json({ message: "ID de usuário inválido." }); }
         const follows = await prisma.follow.findMany({
...
router.get('/:id/likers', checkAuth, async (req, res) => {
    // ... (Código GET /:id/likers original - sem alteração) ...
    try {
         const userId = parseInt(req.params.id, 10);
         if (isNaN(userId)) { return res.status(400).json({ message: "ID de usuário inválido." }); }
...
router.post('/congelar', checkAuth, async (req, res) => {
    // ... (Código POST /congelar original - sem alteração) ...
    const userId = req.user.userId;
    try {
...
router.delete('/excluir', checkAuth, async (req, res) => {
    // ... (Código DELETE /excluir original - sem alteração) ...
    const userId = req.user.userId;
    try {
