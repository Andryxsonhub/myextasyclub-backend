...
// --- CÓDIGO ATUALIZADO (Adicionado Notificações com Deep Link) ---
...
// ROTA 1: Seguir / Deixar de Seguir (Toggle) com Notificação
    ...
    if (existingFollow) {
      ...
      await prisma.notification.create({
        data: {
          ...
          link: `/perfil/${followerId}` // Leva direto para o perfil de quem seguiu
        }
      });
...
// ROTA 2: Curtir / Descurtir Perfil (Toggle) com Notificação
router.post('/profile/:id/like', checkAuth, async (req, res) => {
  ...
  try {
    ...
    if (existingLike) {
      ...
      count = await prisma.like.count({ where: { likedUserId: likedUserId } });
      res.status(200).json({ isLikedByMe: false, likeCount: count });
    } else {
      await prisma.like.create({ data: { likerId, likedUserId } });

      // ★ NOTIFICAÇÃO: CURTIDA NO PERFIL
      await prisma.notification.create({
        data: {
          userId: likedUserId,
          type: 'LIKE',
          content: `Alguém curtiu seu perfil!`,
          link: `/perfil/${likerId}` // Leva para o perfil de quem deu o like
        }
      });
...
// ROTA 3: Buscar estatísticas (Sem alterações na lógica de contagem)
router.get('/me/stats', checkAuth, async (req, res) => {
  ...
  try {
    ...
    const profileLikesCount = await prisma.like.count({ where: { likedUserId: userId } });
    ...
    const photoLikesCount = await prisma.like.count({ where: { likedPhotoId: { in: userPhotoIds } } });
    const videoLikesCount = await prisma.like.count({ where: { likedVideoId: { in: userVideoIds } } });
...
// ROTA 5: Bloquear / Desbloquear um usuário
